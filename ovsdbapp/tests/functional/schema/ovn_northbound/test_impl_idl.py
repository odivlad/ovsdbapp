...
class TestLogicalRouterOps(OvnNorthboundTest):
    ...
    def test_lr_policy_add_allow(self):
        priority = 10
        match = 'ip4.dst==10.1.0.0/16 && ip4.src==10.2.0.0/16'
        action = const.POLICY_ACTION_ALLOW
        policy = self._lr_policy_add(priority, match, action)
    ...
    def test_lr_policy_add_reroute(self):
        priority = 10
        match = 'ip4.dst==10.1.0.0/16 && ip4.src==10.2.0.0/16'
        action = const.POLICY_ACTION_REROUTE
        nexthop = "10.3.0.2"
        policy = self._lr_policy_add(priority, match, action, nexthop=nexthop)
    ...
    def test_lr_policy_add_nexthop_not_reroute(self):
        ...
        nexthop = "10.3.0.2"
        self.assertRaises(ValueError, self._lr_policy_add,
                          priority, match, action, nexthop=nexthop)

    def test_lr_policy_may_exist(self):
        priority = 10
        match = 'ip4.dst==10.1.0.0/16 && ip4.src==10.2.0.0/16'
        action = const.POLICY_ACTION_ALLOW
        policy_init = self._lr_policy_add(priority, match, action)
        self.assertEqual(policy_init.nexthop, [])

        # Update the policy
        action = const.POLICY_ACTION_REROUTE
        nexthop = "10.3.0.2"
        policy_new = self._lr_policy_add(priority, match, action,
                                         nexthop=nexthop, may_exist=True,
    ...
    def _three_policies(self):
        ...
        pr1 = 10
        pr2 = 9
        match1 = 'ip4.src==10.1.1.0/24 && ip4.dst==20.1.1.0/24'
        match2 = 'ip4.src==30.1.1.0/24 && ip4.dst==40.1.1.0/24'
        self._lr_policy_add(pr1, match1, const.POLICY_ACTION_ALLOW,
                            router=lr)
        self._lr_policy_add(pr2, match2, const.POLICY_ACTION_DROP,
                            router=lr)
        self._lr_policy_add(pr1, match2, const.POLICY_ACTION_REROUTE,
                            nexthop='31.1.1.2', router=lr)
        return lr
    ...
    def test_lr_policy_del_match(self):
        match = 'ip4.src==30.1.1.0/24 && ip4.dst==40.1.1.0/24'
        lr = self._lr_policy_del(match=match)
        self.assertEqual(len(lr.policies), 1)
        self.assertEqual(lr.policies[0].action, const.POLICY_ACTION_ALLOW)
    ...
    def test_lr_policy_del_specific(self):
        priority = 10
        match = 'ip4.src==30.1.1.0/24 && ip4.dst==40.1.1.0/24'
        lr = self._lr_policy_del(match=match, priority=priority)
